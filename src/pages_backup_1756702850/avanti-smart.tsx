...
export default function AvantiSmartPage() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const r = await fetch("/api/avanti-smart", {
        method: "POST",
      ...
      <main className="flex-1 overflow-hidden">
        <div ref={scrollRef} className="h-full overflow-auto p-4 space-y-3">
          {messages.filter(m => m.role !== "system").map((m, i) => (
            <div key={i} className={`max-w-[85%] rounded-2xl px-4 py-2 text-sm leading-6 ${m.role === "user" ? "ml-auto bg-neutral-900 text-white" : "mr-auto bg-neutral-100"}`}>
              {m.content}
            </div>
          ))}
